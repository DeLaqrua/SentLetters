...
begin
  ...
  stringgridMails.Cells[2,0] := 'Дата и примерное время отправки:';
  ...
  if FileExists(ExtractFilePath(ParamStr(0))+'codeMO.xls') = True then
    begin
      ...
      try
        ...
        stringgridMails.RowCount := lastRow; //Определяем количество строк StringGrid
    ...
    begin
      if comboboxSelectMonth.ItemIndex = 0 then
        ...
        begin
          j := 0;
          if FindFirst(directoryRoot + 'Archive\' + IntToStr(selectedYear) + '\' +
                       comboboxSelectMonth.Text + '\*.*', faNormal, searchResult) = 0 then
            begin
              repeat
                j := j + 1;
                stringgridMails.Cells[2, j] := DateTimeToStr(FileDateToDateTime(searchResult.Time));
